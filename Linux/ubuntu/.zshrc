...
venv(){
	...
	if [ "$1" = "activate" ]; then
		if (( $#name )); then
			source "${(q+)name[-1]}/bin/activate"
			return
		fi
	elif [ "$1" = "create" ]; then
		if (( $#name )); then
			python -m venv "${(q+)name[-1]}"
			return
		fi
	fi
}
...
alias lsl="ls -lh"
alias lsa="lsl -A"
alias lst="lsa --tree -I node_modules"
alias gcommit="~/.scripts/conventional_commits"
alias untar="tar -xf"
...
export PATH="/home/pablo/.config/composer/vendor/bin:$PATH"
